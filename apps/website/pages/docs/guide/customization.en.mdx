...
> The 50~1100 scale works fine in both "light" or "dark" mode.
> For example: the app background color is controled by var(--sira-colors-bw-50) css varible which will be "#000" in dark mode, but the var(--sira-colors-bw-50) css variable will be "#FFF" in light mode.
...
<div className="grid grid-cols-2 xl:grid-cols-5 gap-4 p-4 has-border rounded-xl">
  <h1 className="text-3xl col-span-full">Built-in Colors</h1>
...
To add a prefix to each Sira components class you can use the `prefix` string property:
